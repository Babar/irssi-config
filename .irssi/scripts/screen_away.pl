...
my $sendnotify = Irssi::settings_get_bool( 'hilightwin_sendnotify' );
...
sub screen_away {
  ...
  if (Irssi::settings_get_bool($IRSSI{'name'} . '_active') == 1) {
    ...
    if ($away == 1 and $away_status != 1) {
      ...
      $away_status = $away;
      $sendnotify = Irssi::settings_get_bool( 'hilightwin_sendnotify' );
      Irssi::settings_set_bool( 'hilightwin_sendnotify', 0 );
      ...
      $away_status = $away;
      Irssi::settings_set_bool( 'hilightwin_sendnotify', $sendnotify );
      Irssi::signal_emit( 'setup changed' );
    }
  }
